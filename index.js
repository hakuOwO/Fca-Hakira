...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/Anupx0/Fca-Horizon-Anup/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-horizon-mod/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-ANUP ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-horizon-anup/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-ANUP ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install Fca-Horizon-Anup@latest', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-horizon-anup/package.json')).version;
    console.clear();
